  ...
  Future<void> _loadContacts() async {
    try {
      ...
      final items = contacts
          .expand((c) => c.phones.map((p) => _ContactItem(
                name: c.displayName,
                rawNumber: p.number,
              )))
  ...
  void _onSearch(String query) {
    ...
    setState(() {
      _filtered = _allContacts!
          ...
          .where((item) =>
              item.rawNumber.isNotEmpty &&
              (q.isEmpty ||
                  item.name.toLowerCase().contains(q) ||
                  item.rawNumber.contains(q)))
          .toList();
    });
  }
  ...
  Widget _buildBody(ScrollController scrollCtrl) {
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.person_search_rounded,
                color: AppColors.textTertiary.withValues(alpha: 0.5), size: 48),
            const SizedBox(height: 12),
    ...
    return ListView.separated(
      ...
      separatorBuilder: (_, __) => const Divider(
          height: 1, indent: 72, endIndent: 16, color: AppColors.border),
...
class _ContactTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avatarColor = network != null
        ? networkColor(network)
        : AppColors.primary;

    final initials = item.name.trim().isEmpty
        ? '#'
    ...
    return ListTile(
      ...
      subtitle: item.name.isEmpty
          ? null
          : Text(item.rawNumber, style: AppTextStyles.bodySmall),
      trailing: network != null
          ? _NetworkTag(network: network)
          : null,
    );
  }
}
